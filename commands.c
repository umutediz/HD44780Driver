...
void _wait(uint32_t ms) { usleep(ms * 1000); }
void _waitMicros(uint32_t micros) { usleep(micros); }
  ...
  switch (lcd->interface) {
    case LCD_I2C_PCF8574:
      if (lcd->busWidth == LCD_BITS_4) {
        ...
        tmpreg = (((reg & 0x0F) << 4) | (reg & 0x100) | (reg & 0x200)) |
                 ((lcd->backlight) << 3);
        _i2cWrite(&(lcd->_i2cHandler), lcd->_i2cAddr, __CONVERT_TO_PCF(tmpreg));
        _waitMicros(5000);
      } else {
...
tLCDReturn configI2C(tLCD* lcd, uint8_t addr) {
  lcd->_i2cAddr = addr;
  // Trigger a dummy read to check if interface is working
  uint8_t dummy;
  _i2cInit(&(lcd->_i2cHandler), lcd->_i2cAddr);
  return readReg(lcd, 0, &dummy);
}
...
tLCDReturn LCDInit(tLCD* lcd, tLCDInterface interface, tBusWidth width,
  ...
  if (rowCount <= MAX_ROW && columnCount <= MAX_COL) {
    ...
    _i2cWrite(
        &(lcd->_i2cHandler), lcd->_i2cAddr,
        __CONVERT_TO_PCF(generateFunctionSet(LCD_BITS_8, rowCount, FS_FONT_8)));
    _wait(5);
    _i2cWrite(
        &(lcd->_i2cHandler), lcd->_i2cAddr,
        __CONVERT_TO_PCF(generateFunctionSet(LCD_BITS_8, rowCount, FS_FONT_8)));
    ...
    _i2cWrite(
        &(lcd->_i2cHandler), lcd->_i2cAddr,
        __CONVERT_TO_PCF(generateFunctionSet(LCD_BITS_4, rowCount, FS_FONT_8)));
    _wait(5);

    state &= writeReg(
        lcd, generateEntryMode(true, false));  // Increment but do not shift
    _waitMicros(50);
    state &= writeReg(
        lcd, generateDisplayConfig(lcd->enable, lcd->showCursor, lcd->blink));
    _waitMicros(50);
    state &=
        writeReg(lcd, generateFunctionSet(lcd->busWidth, lcd->row, FS_FONT_8));
...
tLCDReturn setText(tLCD* lcd, uint8_t row, uint8_t* text, uint8_t length) {
  ...
  if ((text != 0) && (length != 0) && (length <= (MAX_COL)) &&
      (row <= lcd->row)) {
